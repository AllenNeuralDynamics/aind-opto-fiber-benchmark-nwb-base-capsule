...
def create_event_and_meanings_dataframes(data_directory: Path, session_metadata: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    if not stim_csv_path:
        raise FileNotFoundError(f"No stim csv found in directory {data_directory}. Check data")
    
    stim_df = pd.read_csv(stim_csv_path[0])
    stim_metadata = session_metadata["stimulus_epochs"][0]["stimulus_parameters"][0]
    ...
    for num_train in range(int(number_pulse_trains - 1)):
        frame_offset = int(start_frame_onset + (pulse_frequency * pulse_duration))
        event_table_dict["timestamp"].append(stim_df["SoftwareTS"].iloc[frame_offset])
        event_table_dict["event"].append("OptoStimLaser_offset")

        start_frame_onset = int(frame_offset + (pulse_frequency * pulse_interval))
        event_table_dict["timestamp"].append(stim_df["SoftwareTS"].iloc[start_frame_onset])
        event_table_dict["event"].append("OptoStimLaser_onset")
